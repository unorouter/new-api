...
func setupJevRelayTest(t *testing.T) (*model.User, *model.Token) {
	...
	// Prod carries no builtin jev price; the sync writes it, the test writes it here.
	require.NoError(t, config.GlobalConfig.LoadFromDB(map[string]string{
		"billing_setting.billing_mode":    `{"jev-latest":"tiered_expr"}`,
		"billing_setting.billing_expr":    `{"jev-latest":"tier(\"standard\", p * 0.042)"}`,
...
func createJevChannel(t *testing.T, baseURL, key string, passthrough bool, upstreamPath string) *model.Channel {
	...
	require.NoError(t, model.UpdateChannelKey(channel.Id, key))
...
func TestJevChannelManagement(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			adminRouter := apidto.NewRouter(newOpenAPIEngine(), engine, "test")
			apidto.Get(adminRouter, "/models/:id", controller.FetchUpstreamModels)
			apidto.GetP(adminRouter, "/test/:id", controller.TestChannel)
