...
func SetRelayRouter(router *gin.Engine, engine *fuego.Engine) {
	...
	r.GinPost("/messages/count_tokens", controller.CountClaudeTokens)
	...
	// alpha search route (Codex standalone web search)
	r.GinPost("/alpha/search", RelayAlphaSearch)

	// response related routes
	// POST /v1/responses is registered by SetTaskPluginProtocolRouter, which
	// falls through to this same relay path when no plugin is pinned. Registering
	...
	// gemini relay routes
	r.GinPost("/engines/:model/embeddings", RelayEngineEmbeddings, dto.GinResp[relaydto.EmbeddingResponse]())
