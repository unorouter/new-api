...
func AppendClientAttribution(ctx *gin.Context, other *model.LogOther) {
	...
	if title != "" {
		other.SetPublic("client_title", truncateAttribution(title))
	}
	if referer != "" {
		other.SetPublic("client_referer", truncateAttribution(referer))
	}
	if ua != "" {
	...
	if origin, err := common.NormalizeOrigin(h.Get("Origin")); err == nil {
		other.SetPublic("client_origin", truncateAttribution(origin))
	}
}
