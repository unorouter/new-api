...
// An empty-string default is the one literal whose stored form keeps its cast
// through a driver that strips casts by word boundary, so it compares unequal to
// the model forever and re-alters on every startup.
type migrationEmptyDefaultV1 struct {
	ID    int    `gorm:"primaryKey"`
	Label string `gorm:"size:64;default:''"`
	Note  string `gorm:"type:varchar(128);default:''"`
}
	...
	for _, dialect := range []string{"sqlite", "mysql", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			t.Run("empty_string_default", func(t *testing.T) {
				const table = "migration_empty_default_test"
				t.Cleanup(func() { _ = db.Migrator().DropTable(table) })
				require.NoError(t, db.Table(table).AutoMigrate(&migrationEmptyDefaultV1{}))
				require.NoError(t, db.Table(table).Create(&migrationEmptyDefaultV1{ID: 1}).Error)
				recorder.reset()
				require.NoError(t, db.Table(table).AutoMigrate(&migrationEmptyDefaultV1{}))
				assert.Empty(t, recorder.schemaMutations())

				for _, column := range []string{"label", "note"} {
					types, err := db.Table(table).Migrator().ColumnTypes(&migrationEmptyDefaultV1{})
					require.NoError(t, err)
					for _, columnType := range types {
						...
						assert.True(t, ok)
						assert.Empty(t, value, "an empty-string default must normalize to the model value, not keep its cast")
					}
				}
			})
