...
func requestPolicyDefaultOptions() map[string]string {
	...
	defaults["MaxTotalRelayAttempts"] = strconv.Itoa(common.MaxTotalRelayAttempts)
...
func BuildRequestPolicy(options map[string]string) (*RequestPolicySnapshot, error) {
	...
	if attempts, attemptsErr := strconv.Atoi(raw["MaxTotalRelayAttempts"]); attemptsErr != nil || attempts < 1 {
		return nil, fmt.Errorf("max total relay attempts must be a positive integer")
	}
