...
// ping history so pre-existing components are not mis-hidden as week-dead
// right after the deploy. Idempotent; touches only rows still at 0.
func backfillModelStatusLastUp() {
	var cnt int64
	if err := DB.Model(&ModelStatusComponent{}).
		Where("last_up_at = 0").Count(&cnt).Error; err != nil || cnt == 0 {
		return
	}
...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&ChannelDiagnostic{}, "ChannelDiagnostic"},
...
func checkMySQLChineseSupport(db *gorm.DB) error {
	...
	if !isChineseCapable(schemaCharset, schemaCollation) {
		return fmt.Errorf("Schema default charset/collation is not Chinese-capable: schema(%s/%s). Please set the database to utf8mb4/utf8/gbk/big5/gb18030",
	...
	if len(badTables) > 0 {
		...
		return fmt.Errorf(
			"Found tables that are not Chinese-capable. Please fix their collation/charset. Examples (showing up to %d): %v",
			maxShow, shown,
		)
	}
	return nil
