...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	isStream bool, group string, promptTokens int, other *LogOther) {
	...
	log := &Log{
		...
		Type:      LogTypeError,
		Content:   content,
		// Input size of the failed request. CompletionTokens stays 0 because a
		// failure produced no output, and Quota stays 0 because nothing is
		// charged; both token sums are scoped to consume logs, so neither this
		// row nor its tokens reach billing or the tokens-served total.
...
func RecordConsumeLog(c *gin.Context, userId int, params RecordConsumeLogParams) {
	...
	if common.DataExportEnabled {
		...
		otherSnapshot := params.Other.Snapshot()
		if cacheTokens, ok := otherSnapshot["cache_tokens"].(int); ok {
			totalTokens += cacheTokens
		}
		if cacheCreationTokens, ok := otherSnapshot["cache_creation_tokens"].(int); ok {
