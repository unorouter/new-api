...
func readModelPricingMaps(db *gorm.DB) (map[string]map[string]any, map[string]bool, []string, error) {
	var rows []Option
	keys := make([]any, 0, len(modelPricingOptionKeys))
	for _, key := range modelPricingOptionKeys {
		keys = append(keys, key)
	}
	// clause.IN quotes the reserved column per dialect, so this works before
	// initCol has run (tests) and on every database.
