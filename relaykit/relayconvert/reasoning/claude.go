...
func RenderClaude(model string, intent Intent, maxTokens *uint, adapterBudgetPercentage float64) (ClaudeRender, error) {
	...
	if intent.Mode == ModeDisabled || intent.Effort == EffortNone {
		if !capabilities.supportsDisable {
			...
			// instead of refusing it (clients send this for every model). Upstream
			// renders the lowest thinking mode here; prod keeps the default so the
			// request shape stays exactly what the model documents.
			diagnostics = append(diagnostics, claudeReasoningDiagnostic(
				"claude_thinking_disable_unsupported",
				fmt.Sprintf("model %q cannot disable thinking; using the model default", model),
			))
			return ClaudeRender{ClearSampling: capabilities.strictSampling, Diagnostics: diagnostics}, nil
		}
		return ClaudeRender{
