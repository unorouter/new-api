...
// that an oversized tiered settlement clamps to the quota bound instead of
...
func TestComputeTieredQuota_ClampOnOverflow(t *testing.T) {
	...
	// which far exceeds MaxQuota and must saturate.
	...
	assert.Equal(t, common.MaxQuota, result.ActualQuotaAfterGroup, "oversized quota must clamp to the quota bound, never wrap negative")
