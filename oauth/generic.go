...
func validateAccessPolicy(policy *accessPolicy) error {
	if policy == nil {
		return errors.New(i18n.Translate("oauth.policy_is_nil"))
	}
	...
	if len(policy.Conditions) == 0 && len(policy.Groups) == 0 {
		return errors.New(i18n.Translate("oauth.policy_requires_at_least_one_condition_or_group"))
	}
...
func validateAccessCondition(condition *accessCondition, index int) error {
	...
	if condition.Field == "" {
		return fmt.Errorf(i18n.Translate("oauth.condition_field_is_required"), index)
	}

	condition.Op = normalizePolicyOp(condition.Op)
	if !lo.Contains(supportedAccessPolicyOps, condition.Op) {
		return fmt.Errorf(i18n.Translate("oauth.condition_op_is_unsupported"), index, condition.Op)
	}

	if lo.Contains([]string{"in", "not_in"}, condition.Op) {
		if _, ok := condition.Value.([]any); !ok {
			return fmt.Errorf(i18n.Translate("oauth.condition_value_must_be_an_array_for_op"), index, condition.Op)
		}
	}
