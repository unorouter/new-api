...
func SubscriptionRequestWaffoPancakePay(c *gin.Context) {
	...
	if err := order.Insert(); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Failed to create Waffo Pancake subscription order user_id=%d plan_id=%d trade_no=%s error=%q", userId, plan.Id, tradeNo, err.Error()))
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Failed to create Waffo Pancake subscription checkout session user_id=%d plan_id=%d trade_no=%s error=%q", userId, plan.Id, tradeNo, err.Error()))
