...
func GetCustomOAuthProvider(c fuego.ContextNoBody) (*dto.Response[dto.CustomOAuthProviderResponse], error) {
	...
	if err != nil {
		return dto.Fail[dto.CustomOAuthProviderResponse]("Invalid ID")
	}

	provider, err := model.GetCustomOAuthProviderById(id)
	if err != nil {
		return dto.Fail[dto.CustomOAuthProviderResponse]("Custom OAuth provider not found")
	}
...
func FetchCustomOAuthDiscovery(c fuego.ContextWithBody[dto.FetchCustomOAuthDiscoveryRequest]) (*dto.Response[dto.FetchDiscoveryData], error) {
	...
	req, err := c.Body()
	if err != nil {
		return dto.Fail[dto.FetchDiscoveryData]("Invalid request parameters: " + err.Error())
	}
	...
	if wellKnownURL == "" && issuerURL == "" {
		return dto.Fail[dto.FetchDiscoveryData]("Please fill in Discovery URL or Issuer URL first")
	}
	...
	if err != nil || parsedURL.Host == "" || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") {
		return dto.Fail[dto.FetchDiscoveryData]("Discovery URL is invalid, only http/https supported")
	}
	...
	if err != nil {
		return dto.Fail[dto.FetchDiscoveryData]("Failed to create Discovery request: " + err.Error())
	}
	...
	if err != nil {
		return dto.Fail[dto.FetchDiscoveryData]("Failed to get Discovery configuration: " + err.Error())
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return dto.Fail[dto.FetchDiscoveryData]("Failed to get Discovery configuration: " + message)
	}

	var discovery map[string]any
	if err = common.DecodeJson(resp.Body, &discovery); err != nil {
		return dto.Fail[dto.FetchDiscoveryData]("Failed to parse Discovery configuration: " + err.Error())
	}
...
func CreateCustomOAuthProvider(c fuego.ContextWithBody[dto.CreateCustomOAuthProviderRequest]) (*dto.Response[dto.CustomOAuthProviderResponse], error) {
	req, err := c.Body()
	if err != nil {
		return dto.Fail[dto.CustomOAuthProviderResponse]("Invalid request parameters: " + err.Error())
	}

	// Check if slug is already taken
	if model.IsSlugTaken(req.Slug, 0) {
		return dto.Fail[dto.CustomOAuthProviderResponse]("Slug already exists")
	}

	// Check if slug conflicts with built-in providers
	if oauth.IsProviderRegistered(req.Slug) && !oauth.IsCustomProvider(req.Slug) {
		return dto.Fail[dto.CustomOAuthProviderResponse]("This slug conflicts with a built-in OAuth provider")
	}
	...
	return dto.OkMsg("Creation successful", *toCustomOAuthProviderResponse(provider))
}
...
func UpdateCustomOAuthProvider(c fuego.ContextWithBody[dto.UpdateCustomOAuthProviderRequest]) (*dto.Response[dto.CustomOAuthProviderResponse], error) {
	...
	return dto.OkMsg("Update successful", *toCustomOAuthProviderResponse(provider))
}
...
func DeleteCustomOAuthProvider(c fuego.ContextNoBody) (dto.MessageResponse, error) {
	...
	provider, err := model.GetCustomOAuthProviderById(id)
	if err != nil {
		return dto.FailMsg("Custom OAuth provider not found")
	}
	...
	if err != nil {
		...
		return dto.FailMsg("Error checking user bindings, please try again later")
	}
	if count > 0 {
		return dto.FailMsg("This OAuth provider still has user bindings and cannot be deleted. Please unbind all users first.")
	}
	...
	return dto.Msg("Deletion successful")
}
...
func GetUserOAuthBindings(c fuego.ContextNoBody) (*dto.Response[[]dto.UserOAuthBindingResponse], error) {
	userId := dto.UserID(c)
	if userId == 0 {
		return dto.Fail[[]dto.UserOAuthBindingResponse]("Not logged in")
	}
...
func GetUserOAuthBindingsByAdmin(c fuego.ContextNoBody) (*dto.Response[[]dto.UserOAuthBindingResponse], error) {
	...
	if err != nil {
		return dto.Fail[[]dto.UserOAuthBindingResponse]("Invalid ID")
	}
	...
	if !canManageTargetRole(myRole, targetUser.Role) {
		return dto.Fail[[]dto.UserOAuthBindingResponse]("No permission to access users of same or higher level")
	}
