...
func GetCheckinStatus(c fuego.ContextWithParams[dto.GetCheckinStatusParams]) (*dto.Response[dto.CheckinStatusData], error) {
	...
	if !setting.Enabled {
		return dto.Fail[dto.CheckinStatusData](common.TranslateMessage(dto.GinCtx(c), "checkin.disabled"))
	}
	userId := dto.UserID(c)
...
func DoCheckin(c fuego.ContextNoBody) (*dto.Response[dto.CheckinResultData], error) {
	setting := operation_setting.GetCheckinSetting()
	if !setting.Enabled {
		return dto.Fail[dto.CheckinResultData](common.TranslateMessage(dto.GinCtx(c), "checkin.disabled"))
	}
	...
	return dto.OkMsg(common.TranslateMessage(dto.GinCtx(c), "checkin.success"), dto.CheckinResultData{
