...
func verifyAPITokenAudit(t *testing.T) {
	...
	tokenRoutes.POST("/", fuegoWithBody(AddToken))
	tokenRoutes.PUT("/", fuegoBodyParams(UpdateToken))
	tokenRoutes.DELETE("/:id", fuegoNoBody(DeleteToken))
	tokenRoutes.POST("/batch", fuegoWithBody(DeleteTokenBatch))
	...
	tokenRoutes.POST("/:id/key", func(c *gin.Context) {
		...
		c.Next()
	}, fuegoNoBody(GetTokenKey))
	tokenRoutes.GET("/", fuegoNoBody(GetAllTokens))
	tokenRoutes.GET("/:id", fuegoNoBody(GetToken))
	tokenRoutes.GET("/search", fuegoWithParams(SearchTokens))
	...
	t.Run("bounded batch metadata", func(t *testing.T) {
		...
		body, err := common.Marshal(dto.TokenBatch{Ids: ids})
		require.NoError(t, err)
