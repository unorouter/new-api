...
// GenerateAccessTokenVerified is the built-in frontend's proof-gated variant.
// The BFF keeps calling the legacy GET handler in user.go without a proof.
func GenerateAccessTokenVerified(c *gin.Context) {
	...
	// Privileged accounts administer through a console session; no PAT is ever
	// minted for them (see GenerateAccessToken).
