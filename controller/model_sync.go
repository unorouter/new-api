...
func SyncUpstreamModels(c fuego.ContextWithBody[dto.SyncRequest]) (*dto.Response[dto.SyncUpstreamResult], error) {
	...
	if err != nil {
		...
		return dto.Fail[dto.SyncUpstreamResult](common.TranslateMessage(dto.GinCtx(c), "sync.get_model_list_failed"))
	}
	...
	wg.Wait()
	if fetchErr != nil {
		return dto.Fail[dto.SyncUpstreamResult](common.TranslateMessage(dto.GinCtx(c), "sync.get_upstream_model_failed", map[string]any{"Error": fetchErr.Error()}))
	}
...
func SyncUpstreamPreview(c fuego.ContextWithParams[dto.SyncUpstreamPreviewParams]) (*dto.Response[dto.SyncPreviewResult], error) {
	...
	wg.Wait()
	if fetchErr != nil {
		return dto.Fail[dto.SyncPreviewResult](common.TranslateMessage(dto.GinCtx(c), "sync.get_upstream_model_failed", map[string]any{"Error": fetchErr.Error()}))
	}
