...
func DeleteHistoryLogs(c fuego.ContextWithParams[dto.DeleteHistoryLogsParams]) (*dto.Response[dto.DeletedCountData], error) {
	p, _ := dto.ParseParams[dto.DeleteHistoryLogsParams](c)
	if p.TargetTimestamp == 0 {
		return dto.Fail[dto.DeletedCountData]("target timestamp is required")
	}
	count, err := model.DeleteOldLog(c.Request().Context(), p.TargetTimestamp, 100)
	if err != nil {
		return dto.Fail[dto.DeletedCountData](err.Error())
	}
	return dto.Ok(dto.DeletedCountData{Deleted: count})
}
