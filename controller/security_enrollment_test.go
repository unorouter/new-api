...
func TestSecurityEnrollmentAccessTokenRequiresProofBeforeMutation(t *testing.T) {
	...
	for _, endpoint := range []struct {
		...
		{"GET", GenerateAccessTokenVerified},
		{"POST", GenerateAccessTokenVerified},
...
func TestSecurityEnrollmentAccessTokenLifecycleConsumesProofs(t *testing.T) {
	...
	for _, method := range []string{"GET", "POST"} {
		...
		response := securityEnrollmentRequest(method, "/api/user/token", "", proof.ProofToken, identity, GenerateAccessTokenVerified)
		var body securityEnrollmentResponse
		require.NoError(t, common.Unmarshal(response.Body.Bytes(), &body))
	...
	// Prod also records every refused proof (security.proof_rejected); only the
	// access-token operations are under test here.
	require.NoError(t, model.LOG_DB.Where("action LIKE ?", "access_token.%").Find(&audits).Error)
...
func TestSecurityEnrollmentAccessTokenRejectsInvalidProofs(t *testing.T) {
	...
	for _, endpoint := range []struct {
		...
		{"GET", service.VerificationScopeAccessTokenGenerate, GenerateAccessTokenVerified},
		{"POST", service.VerificationScopeAccessTokenGenerate, GenerateAccessTokenVerified},
...
func TestSecurityEnrollmentProofRequiresLiveRecordAndExactDeadline(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, service.SecurityProofTTL.Seconds(), claims["exp"].(float64)-claims["iat"].(float64))
...
func TestSecurityEnrollmentPasskeyProofProtectsChannelKeyRead(t *testing.T) {
	...
	router := gin.New()
	router.POST("/api/channel/:id/key", middleware.RootAuth(), middleware.SecureVerificationRequired(), fuegoNoBody(GetChannelKey))
	for _, test := range []struct {
...
func TestSecurityEnrollmentMissingTargetsAreBusinessErrors(t *testing.T) {
	...
	for _, target := range []struct {
		...
		{"/api/channel/999/key", i18n.MsgChannelNotExists, fuegoNoBody(GetChannelKey)},
		...
		if target.key != i18n.MsgChannelNotExists {
			// The fuego channel-key handler keeps prod's 200 + success:false shape.
