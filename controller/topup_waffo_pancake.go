...
func CreateWaffoPancakePair(c *gin.Context) {
	...
	if merchantID == "" || privateKey == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Waffo Pancake credentials are not configured"})
		return
	}
...
func CreateWaffoPancakeSubscriptionProduct(c *gin.Context) {
	...
	if merchantID == "" || privateKey == "" || storeID == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Waffo Pancake setup is incomplete, please complete the gateway binding in payment settings first"})
		return
	}
