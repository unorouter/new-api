...
func CreatePrefillGroup(c fuego.ContextWithBody[model.PrefillGroup]) (*dto.Response[model.PrefillGroup], error) {
	...
	if g.Name == "" || g.Type == "" {
		return dto.Fail[model.PrefillGroup]("Group name and type cannot be empty")
	}
	// 创建前检查名称
	...
	if dup {
		return dto.Fail[model.PrefillGroup]("Group name already exists")
	}
...
func UpdatePrefillGroup(c fuego.ContextWithBody[model.PrefillGroup]) (*dto.Response[model.PrefillGroup], error) {
	...
	if g.Id == 0 {
		return dto.Fail[model.PrefillGroup]("Group ID is missing")
	}
	// 名称冲突检查
