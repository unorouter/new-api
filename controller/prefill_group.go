...
func CreatePrefillGroup(c fuego.ContextWithBody[model.PrefillGroup]) (*dto.Response[model.PrefillGroup], error) {
	...
	if g.Name == "" || g.Type == "" {
		return dto.Fail[model.PrefillGroup](common.TranslateMessage(dto.GinCtx(c), "prefill_group.name_type_empty"))
	}
	// 创建前检查名称
	...
	if dup {
		return dto.Fail[model.PrefillGroup](common.TranslateMessage(dto.GinCtx(c), "prefill_group.name_exists"))
	}
...
func UpdatePrefillGroup(c fuego.ContextWithBody[model.PrefillGroup]) (*dto.Response[model.PrefillGroup], error) {
	...
	if g.Id == 0 {
		return dto.Fail[model.PrefillGroup](common.TranslateMessage(dto.GinCtx(c), "prefill_group.id_missing"))
	}
	// 名称冲突检查
