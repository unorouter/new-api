...
func GetAllModelsMeta(c fuego.ContextNoBody) (*dto.Response[ModelsMetaListData], error) {
	...
	syncOfficial := c.QueryParam("sync_official")
...
func SearchModelsMeta(c fuego.ContextWithParams[dto.SearchModelsMetaParams]) (*dto.Response[ModelsMetaListData], error) {
	p, _ := dto.ParseParams[dto.SearchModelsMetaParams](c)
	pageInfo := dto.PageInfo(c)

	modelsMeta, total, err := model.SearchModels(p.Keyword, p.Vendor, p.Status, p.SyncOfficial, pageInfo.GetStartIdx(), pageInfo.GetPageSize())
	if err != nil {
		return dto.Fail[ModelsMetaListData](err.Error())
	}
	// 批量填充附加字段，提升列表接口性能
...
func CreateModelMeta(c fuego.ContextWithBody[model.Model]) (*dto.Response[model.Model], error) {
	...
	if m.ModelName == "" {
		return dto.Fail[model.Model](common.TranslateMessage(dto.GinCtx(c), "model.name_empty"))
	}
	// 名称冲突检查
	if dup, err := model.IsModelNameDuplicated(0, m.ModelName); err != nil {
		return dto.Fail[model.Model](err.Error())
	} else if dup {
		return dto.Fail[model.Model](common.TranslateMessage(dto.GinCtx(c), "model.name_exists"))
	}
...
func UpdateModelMeta(c fuego.Context[model.Model, dto.StatusOnlyParams]) (*dto.Response[model.Model], error) {
	...
	if m.Id == 0 {
		return dto.Fail[model.Model](common.TranslateMessage(dto.GinCtx(c), "model.id_missing"))
	}
