...
func AdminCreateUserSubscription(c fuego.ContextWithBody[dto.AdminCreateUserSubscriptionRequest]) (dto.MessageResponse, error) {
	userId, err := c.PathParamIntErr("id")
	if err != nil || userId <= 0 {
		return dto.FailMsg("无效的用户ID")
	}
	req, err := c.Body()
...
func AdminInvalidateUserSubscription(c fuego.ContextNoBody) (dto.MessageResponse, error) {
	subId, err := c.PathParamIntErr("id")
	if err != nil || subId <= 0 {
		return dto.FailMsg("无效的订阅ID")
	}
...
func AdminDeleteUserSubscription(c fuego.ContextNoBody) (dto.MessageResponse, error) {
	subId, err := c.PathParamIntErr("id")
	if err != nil || subId <= 0 {
		return dto.FailMsg("无效的订阅ID")
	}
