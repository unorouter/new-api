...
func TestShouldRetryHonorsPinRetryMode(t *testing.T) {
	...
	single := newPinRetryContext()
	service.GetChannelConstraints(single).AddPin(dto.ChannelPin{
		ChannelId: 1,
		Source:    dto.PinSourceOriginTask,
		Rank:      dto.PinRankOriginTask + 1,
		RetryMode: dto.PinRetrySingleAttempt,
	})
	assert.Equal(t, service.PolicyDecision{Action: "stop", Reason: "pinned_channel", Source: "channel_constraint"}, service.DecideRelayRetry(single, openaiErr, 1), "a single-attempt pin suppresses retry")
}
...
func TestShouldRetryTaskRelayHonorsPinRetryMode(t *testing.T) {
	...
	assert.Equal(t, service.PolicyDecision{Action: "stop", Reason: "pinned_channel", Source: "channel_constraint"}, decideTaskRetry(single, taskErr, 1))
}
