...
const (
	...
	// Room traffic is bursty by nature: admitting a guest publishes the
	// transcript and then the welcome, and a streaming reply emits a delta per
	// render frame. A minimum gap between frames would drop those, so the limit
	// is a token bucket instead: a burst passes, a sustained flood does not.
	roomBurst    = 64
	roomRefillPS = 20
)

// roomLimiter is a per-connection token bucket. Not safe for concurrent use;
// each connection owns its own and the read loop is single-threaded.
type roomLimiter struct {
	tokens float64
	last   time.Time
}

func (l *roomLimiter) allow() bool {
	now := time.Now()
	if l.last.IsZero() {
		l.tokens = roomBurst
	} else {
		l.tokens += now.Sub(l.last).Seconds() * roomRefillPS
		if l.tokens > roomBurst {
			l.tokens = roomBurst
		}
	}
	l.last = now
