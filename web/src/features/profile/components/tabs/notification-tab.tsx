...
export function NotificationTab({ profile, onUpdate }: NotificationTabProps) {
  ...
  useEffect(() => {
    if (profile?.setting) {
      ...
      setSettings({
        quota_warning_enabled: parsed.quota_warning_enabled || false,
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Quota Warning Master Switch */}
      <div className="flex items-center justify-between gap-4">
        <div className="space-y-0.5">
          <Label htmlFor="quotaWarningEnabled">
            {t("Quota Warning Notifications")}
          </Label>
          <p className="text-muted-foreground text-xs">
            {t("Receive an alert when your balance runs low")}
          </p>
        </div>
        <Switch
          id="quotaWarningEnabled"
          checked={settings.quota_warning_enabled}
          onCheckedChange={(checked) =>
            updateField("quota_warning_enabled", checked)
          }
        />
      </div>
            ...
            <>
              ...
              <div className="bg-muted/50 rounded-lg border p-3 sm:p-4">
                ...
                <ol className="text-muted-foreground space-y-1 text-xs">
                  ...
                  <li>
                    {t("3. Enter your Gotify server URL and token above")}
                  </li>
                </ol>
