...
describe('pricing controls', () => {
  it('counts each model once per filter and updates counts when the catalog changes', () => {
    ...
    // Empty quota buckets are dropped from the sidebar, so only the matching
    // bucket and All Models survive a catalog that lost every other type.
    expect(screen.queryByRole('button', { name: /^Token-based\s*0$/ })).toBeNull()
    expect(screen.queryByRole('button', { name: /^Task billing\s*0$/ })).toBeNull()
    expect(
      screen.getByRole('button', { name: /^Per Request\s*1$/ })
    ).toBeVisible()
  })
  ...
  // Prod's sidebar renders groups with its own online/offline markers; the
  // mobile-filter case targets upstream's group buttons.
})
