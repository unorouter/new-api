...
export const PricingSidebar = memo(function PricingSidebar(
  ...
  const counts = useMemo(() => {
    ...
    const quotas = { token: 0, request: 0, task: 0, custom: 0, grid: 0 }
    for (const model of props.models) {
      ...
      if (hasTaskUsageSchema(model)) {
        ...
        quotas.request++
      } else if (model.quota_type === 3) {
        quotas.custom++
      } else if (model.quota_type === 4) {
        quotas.grid++
      }
    }
