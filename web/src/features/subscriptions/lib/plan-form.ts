...
export function getPlanFormSchema(t: TFunction) {
  return z.object({
    ...
    free_rate_limit_window_pct: z.coerce.number().min(0).max(100),
...
export function planToFormValues(plan: SubscriptionPlan): PlanFormValues {
  return {
    ...
    free_rate_limit_window_pct: Number(plan.free_rate_limit_window_pct || 0),
...
export function formValuesToPlanPayload(values: PlanFormValues): PlanPayload {
  return {
    plan: {
      ...
      free_rate_limit_window_pct: Number(values.free_rate_limit_window_pct || 0),
