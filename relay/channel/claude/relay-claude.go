...
// claudeResponsesStreamStateKey stores the prod-only responses-via-chat stream
// state on the request context so it survives across streaming chunks and the
// final-response call for the same request.
const claudeResponsesStreamStateKey = "claude_responses_stream_state"

func getClaudeResponsesStreamState(c *gin.Context, claudeInfo *ClaudeResponseInfo) *openaicompat.ChatToResponsesStreamState {
	if v, ok := c.Get(claudeResponsesStreamStateKey); ok {
		if state, ok := v.(*openaicompat.ChatToResponsesStreamState); ok && state != nil {
			return state
		}
	}
	state := openaicompat.NewChatToResponsesStreamState(claudeInfo.ResponseId, claudeInfo.Created, claudeInfo.Model)
	c.Set(claudeResponsesStreamStateKey, state)
	return state
}
...
func HandleStreamResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, data string) *types.NewAPIError {
	...
	if info.RelayFormat == types.RelayFormatClaude {
		...
		streamState := getClaudeResponsesStreamState(c, claudeInfo)
