...
// overflow. Wallet paths use the wallet bound (MaxWalletQuota); the per-request
// bound is MaxInt32, which at QuotaPerUnit=500000 refused every purchase above
// $4294: RechargeCreem returned an error, the webhook answered 500, and the
// buyer was charged without receiving credit.
func TestWalletQuotaFromDecimalStrictAcceptsLargeTopUps(t *testing.T) {
...
func TestWalletQuotaFromDecimalStrictAcceptsCreemQuotaUnits(t *testing.T) {
	quota, err := WalletQuotaFromDecimalStrict(decimal.NewFromInt(2_500_000_000))
